...
- (void)showImage
{
    if (_photo.firstShow && _photo.url) { // 首次显示
...
- (void)adjustFrame
{
    ...
    CGFloat imageWidth = imageSize.width/_imageView.image.scale;
    CGFloat imageHeight = imageSize.height/_imageView.image.scale;
...
//	}
    ...
    // 内容尺寸
    self.contentSize = CGSizeMake(0, boundsSize.height);
...
- (void)hide
{
    ...
    CGRect r = [_photo.srcImageView.superview convertRect:_photo.srcImageView.frame toView:self];
