...
@implementation CExpandHeader{
    ...
    __weak CCMainHeaderView *_expandView; //背景可以伸展的View
...
+ (id)expandWithScrollView:(UIScrollView*)scrollView expandView:(CCMainHeaderView*)expandView{
...
- (void)scrollViewDidScroll:(UIScrollView*)scrollView
{
    ...
    if(offsetY < _expandHeight * -1) {
        CGRect currentFrame = _expandView.backgroundImageView.frame;
        currentFrame.origin.y = offsetY;
        currentFrame.size.height = -1*offsetY;
        _expandView.backgroundImageView.frame = currentFrame;
    }
